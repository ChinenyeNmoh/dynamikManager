...
const MessageCard = ({ message }) => {
  ...
  const handleReadClick = async () => {
    try {
      const response = await axios.put(`http://localhost:5000/api/messages/read/`, {id: message._id}, {
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:5000/api/messages/${message._id}`, { withCredentials: true });
